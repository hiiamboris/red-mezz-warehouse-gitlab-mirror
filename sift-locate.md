...
| <pre><br>path: back tail path<br>until [<br>    targ: :path/1<br>    if all [<br>        attempt [:targ/feel/over]<br>        targ <> window<br>        any [changed  previndex <> index  find window/face-flags 'all-over]<br>    ][<br>        do-smth...<br>        break<br>    ]<br>    <br>    path: back path<br>    head? path<br>]<br></pre> | <pre><br>if locate/back path [<br>	targ .. :targ/feel/over<br>	targ <> window<br>	any [changed  previndex <> index  find window/face-flags 'all-over]<br>][<br>	do-smth..<br>]<br></pre> | Non-essential code is kept at minimum |
...
| `extract remove-each [role value] any [roles []] [value <> "yes"] 2` | `sift any [roles []] [p: role - .. p/2 <> "yes"]` | Extract is thus unnecessary |
