...
| <pre><br>forall objs [<br>	all [<br>		in objs/1 'data<br>		in objs/1/data 'type<br>		objs/1/data/type = 'container<br>		objs: head pos: objs<br>		break<br>	]<br>]<br>if pos [do-smth]<br></pre> | <pre>if pos: locate/back objs [obj .. obj/data/type = 'container] [do-smth]</pre> | Paths are automatically checked for existence - no error dodging required |
...
| <pre>extract remove-each [role value] any [roles []] [value = "yes"] 2</pre> | `sift any [roles []] [p: role - .. p/2 <> "yes"]` | Extract is thus unnecessary |
